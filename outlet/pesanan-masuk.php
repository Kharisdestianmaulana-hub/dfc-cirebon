...
$kode_outlet = $_SESSION['kode_outlet']; // Misal isinya: 'sutawinangun'
...
if(isset($_POST['update_status'])) {
    ...
    mysqli_query($conn, "UPDATE transaksi_pesanan SET status_pesanan='$status_baru' WHERE order_id='$order_id'");
...
<body>
    ...
    <div class="container">
        ...
        <?php
        // ==========================================================
        // PERBAIKAN KUERI: PAKAI LIKE AGAR NAMA OUTLET PASTI KETEMU
        // ==========================================================
        // Mencari semua pesanan di mana kode_outlet mengandung kata 'sutawinangun'
        $query_orders = mysqli_query($conn, "SELECT * FROM transaksi_pesanan WHERE kode_outlet LIKE '%$kode_outlet%' AND status_pesanan != 'Selesai' ORDER BY waktu_pesan ASC");
        
        if(mysqli_num_rows($query_orders) == 0) {
            echo "<div style='background:white; padding:40px; text-align:center; border-radius:12px; color:#888; box-shadow: 0 4px 10px rgba(0,0,0,0.05);'>Belum ada pesanan masuk saat ini.</div>";
        }
